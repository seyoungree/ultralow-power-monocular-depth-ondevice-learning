...
PREDICTOR_NAME = "dummy_depth_predictor_odl_4room.npy"
VARIANCE_NAME  = "variance_depth_matrix_odl_4room.npy"
...
trainset    = dataloader.miniNYUv2(MININYUV2_PATH, transform=False, set='train', normalize=normalize_imgs, flip_horizontally=False)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=1, shuffle=False, num_workers=0)

valset      = dataloader.miniNYUv2(MININYUV2_PATH, transform=False, set='val', normalize=normalize_imgs, flip_horizontally=False)
valloader   = torch.utils.data.DataLoader(valset, batch_size=1, shuffle=False, num_workers=0)

testset     = dataloader.miniNYUv2(MININYUV2_PATH, transform=False, set='test', normalize=normalize_imgs, flip_horizontally=False)
...
# Statistics vs NYUV2 ground truth
...
with torch.no_grad():
    for test_data in testloader:
        ...
        test_out_depth_ups  = transforms.functional.resize(img=test_outputs, size=[test_depthGT.size()[-3], test_depthGT.size()[-2]], interpolation=transforms.InterpolationMode.NEAREST)
...
if VISUAL_TEST == 'Yes':
    ...
    with torch.no_grad():
        ...
        for i in range(6):
            ...
            ax[i][1].title.set_text("Depth Map")
