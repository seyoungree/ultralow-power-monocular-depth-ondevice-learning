...
def ProxySupervisionLoss(output, target, alpha=0.2, invalid_value=-1, device='cpu'):
...
def PhotometricReprojectionLoss(imgL, imgR, disparity, device):
...
def MonocularDepthSemiSupervisedLoss(output, target, imgL, imgR, aap, aps, alpha, invalid_value, original_width, device):
    """
    Parameters:
    ...
    Lap, La, Lb = PhotometricReprojectionLoss(imgL, imgR, output, device)
    Lps         = ProxySupervisionLoss       (output, target, alpha, invalid_value, device)
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(trainloader):
        ...
        disp    = data['dispL'].to(device)
        depth   = data['depthL'].to(device)
