...
parser.add_argument( '--scheduler_epochs_step', type=int, default=20)
...
trainset    = dataloader.miniKITTI(MINIKITTI_PATH, transform=transform_train, set='train', resolution=kitti_source_resolution, normalize=normalize_imgs, flip_horizontally=HFLIP, train_subset_percentage=PERCENTAGE_KITTI_SAMPLES)
...
valset      = dataloader.miniKITTI(MINIKITTI_PATH, transform=transform_val, set='val', resolution=kitti_source_resolution, normalize=normalize_imgs, flip_horizontally=False)
...
for epoch in range(starting_epoch, epochs, 1):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader):
        ...
        # imgL = imgL.to(device) #.type(torch.FloatTensor)
        # imgR = imgR.to(device) #.type(torch.FloatTensor)
        imgL = imgL.to(device)
        imgR = imgR.to(device)
        disp = disp.to(device)
        ...
        with torch.no_grad():
            # print statistics
            # Indices
            check_idx = 40 # ((len(trainset)+1) / batch_size) / 100
