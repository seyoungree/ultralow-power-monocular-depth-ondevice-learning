...
class miniKITTI(Dataset):
    ...
    def __init__(self, root_dir, transform=False, set='train', resolution='48x48', normalize=True, flip_horizontally=True, train_subset_percentage=None, test_subset_percentage=None):
        """
        ...
        if set == 'train':
            ...
            if test_subset_percentage != None and test_subset_percentage < 100 and test_subset_percentage > 0:
                self.dataset_annotations = pd.read_csv(f'{self.root_dir}/{self.split}/annotations_{test_subset_percentage}perc.csv', header=None)
            else:
                self.dataset_annotations = pd.read_csv(f'{self.root_dir}/{self.split}/annotations.csv', header=None)
        else:
