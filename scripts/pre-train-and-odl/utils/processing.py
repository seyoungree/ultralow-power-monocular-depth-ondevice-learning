...
# Compute depth maps from predictions in validation
def compute_depth_map_test(cam_par, disparities, device, data_type='fp32'):
    batch_size, hin, win = disparities.size()

    if data_type == 'fp32':
        dttype = torch.FloatTensor
    elif data_type == 'fp16':
        dttype = torch.HalfTensor
    elif data_type == 'bfloat16':
        dttype = torch.BFloat16Tensor

    cam_par = cam_par.type(dttype).to(device)
