...
parser.add_argument( '--tartanair_path', type=str, default='../../micro-tartanair-hard/')
parser.add_argument( '--model_name', type=str, default='upydnet')                     # 'cnn' or 'upydnet' or 'upydnet_l'
parser.add_argument( '--saved_mdl_path', type=str, default='saved_models/micro-tartanair-pre-train/')
...
parser.add_argument( '--log_dir', type=str, default='./tartanair_distr/')
...
tartanair_scenarios_test   = ['abandonedfactory', 'neighborhood', 'seasonsforest']
#tartanair_scenarios_test = ['abandonedfactory']

testset    = dataloader.miniTartanAir(root_dir=TARTANAIR_PATH, transform=transform_test, scenarios=tartanair_scenarios_test, normalize=normalize_imgs, flip_horizontally=False)
...
print(f"Test set size: {testset.__len__()}")
...
if model_name == 'upydnet':
    ...
    print("\nModel to be trained:")
    summary(net, (IM_CH_IN, IM_H_IN, IM_W_IN), batch_size=1)
...
with torch.no_grad():
    for test_data in testloader:
        ...
        test_Lap  += 0 #test_Lap_t
        test_Lps  += 0 #test_Lps_t
        test_La   += 0 #test_La_t
        test_Lb   += 0 #test_Lb_t
        ...
        test_out_depth_ups = compute_depth_map_validation(test_fb, test_outputs, device)
        run_abs_rel, run_sq_rel, run_rmse, run_rmse_log, run_d1, run_d2, run_d3 = compute_errors_masked_train(test_out_depth_ups.to(device), test_depth.to(device))
        run_silog = ScaleInvariantMSELogLoss(test_out_depth_ups.to(device), test_depth.to(device), 1.0)
        ...
        max_0_15 = 15.0
        run_abs_rel_15, run_sq_rel_15, run_rmse_15, run_rmse_log_15, run_d1_15, run_d2_15, run_d3_15 = compute_errors_masked_train_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), max_0_15)
        run_silog_15 = ScaleInvariantMSELogLoss_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), 1.0, max_0_15)
        ...
        max_0_25 = 25.0
        run_abs_rel_25, run_sq_rel_25, run_rmse_25, run_rmse_log_25, run_d1_25, run_d2_25, run_d3_25 = compute_errors_masked_train_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), max_0_25)
        run_silog_25 = ScaleInvariantMSELogLoss_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), 1.0, max_0_25)
        ...
        max_0_50 = 50.0
        run_abs_rel_50, run_sq_rel_50, run_rmse_50, run_rmse_log_50, run_d1_50, run_d2_50, run_d3_50 = compute_errors_masked_train_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), max_0_50)
        run_silog_50 = ScaleInvariantMSELogLoss_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), 1.0, max_0_50)
        ...
        max_0_80 = 80.0
        run_abs_rel_80, run_sq_rel_80, run_rmse_80, run_rmse_log_80, run_d1_80, run_d2_80, run_d3_80 = compute_errors_masked_train_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), max_0_80)
        run_silog_80 = ScaleInvariantMSELogLoss_max_dpth(test_out_depth_ups.to(device), test_depth.to(device), 1.0, max_0_80)
    ...
    #     f.write(f"abs_rel = {pr_abs_rel:.3f}, sq_rel = {pr_sq_rel:.3f}, rmse = {pr_rmse:.3f}, rmse_log = {pr_rmse_log:.3f}, d1 = {pr_d1:.3f}, d2 = {pr_d2:.3f}, d3 = {pr_d3:.3f}")
    #     f.write(f"\n\n")

    # with open(log_csv, 'a') as fcsv:
    #     fcsv.write(f"proxy_dpth,{pr_abs_rel:.3f},{pr_sq_rel:.3f},{pr_rmse:.3f},{pr_rmse_log:.3f},{pr_d1:.3f},{pr_d2:.3f},{pr_d3:.3f}\n")
    ...
    with open(log_csv, 'a') as fcsv:
        fcsv.write(f"50m,{abs_rel_80:.3f},{sq_rel_80:.3f},{rmse_80:.3f},{rmse_log_80:.3f},{d1_80:.3f},{d2_80:.3f},{d3_80:.3f},{silog_80:.3f}\n")
