...
import utils.kitti_dataloader as dataloader
# Custom loss
...
parser.add_argument( '--dataset_resolution', type=str, default='48x48') # Resolution of the test dataset
parser.add_argument( '--model_name', type=str, default='upydnet')       # 'cnn', 'upydnet' or 'upydnet_l'
parser.add_argument( '--saved_mdl_path', type=str, default='checkpoints/micro-kitti-8x8-label/sparse_update_100percdata/sparse_e0_d01_d10_d20/run2/')
...
parser.add_argument( '--log_dir', type=str, default='./kitti_distr/')
...
parser.add_argument( '--kitti_dummy_predictor_path', type=str, default="./checkpoints/dummy_predictors/micro-kitti/")
parser.add_argument( '--tartanair_dummy_predictor_path', type=str, default="./checkpoints/dummy_predictors/micro-tartanair/")
...
parser.add_argument( '--percentage_kitti_testset', type=int, default=100)        # Specify the percentage of test data to be used to test (simulate lower frequency for ToF during OOD detection)
...
PERCENTAGE_KITTI_SAMPLES = args.percentage_kitti_testset
...
KITTI_DUMMY_PATH = args.kitti_dummy_predictor_path
...
kitti_source_resolution = args.dataset_resolution # '48x48'
normalize_imgs          = True
...
testset     = dataloader.miniKITTI(MINIKITTI_PATH, transform=None, set='test', resolution=kitti_source_resolution, normalize=normalize_imgs, test_subset_percentage=PERCENTAGE_KITTI_SAMPLES)
testloader  = torch.utils.data.DataLoader(testset, batch_size=1, shuffle=False, num_workers=0)

if PERCENTAGE_KITTI_SAMPLES < 100:
    print(f"Testing on a subset of {testset.__len__()} samples ({PERCENTAGE_KITTI_SAMPLES}% of the full mini-kitti test set)")
...
if SCALE_PREDICTION == 'Y':
    kitti_mean_matrix         = torch.from_numpy(np.load(f"{KITTI_DUMMY_PATH}/dummy_depth_predictor.npy")).to(device)
    kitti_variance_matrix     = torch.from_numpy(np.load(f"{KITTI_DUMMY_PATH}/variance_depth_matrix.npy")).to(device)
...
# 0-80 m
test_loss_80 = 0; test_Lap_80 = 0; test_Lps_80 = 0; test_La_80 = 0; test_Lb_80 = 0
abs_rel_80=0; sq_rel_80=0; rmse_80=0; rmse_log_80=0; d1_80=0; d2_80=0; d3_80=0; silog_80=0
avg_num_80m_points = 0
...
with torch.no_grad():
    for test_data in testloader:
        ...
        if SIMULATE_TOF == 'Y':
            if NUM_TOF_MINIMA == 1:
                ...
                test_depthGT = simulate_tof_sensor_ground_truth(test_depthGT, 8, 8, test_depthGT.size()[-2], test_depthGT.size()[-1], 0)
                ...
                test_depthGT = convert_depth_label_to_multi_target_depth_GT(test_depthGT.to(device), NUM_TOF_MINIMA, device, 'fp32').to(device)
        ...
        if SIMULATE_TOF == 'Y' and NUM_TOF_MINIMA > 1:
            ...
            with torch.no_grad():
                data_transf      = PredictionMultitargetFinder(device, 'fp32')
                test_outputs_pr = data_transf(test_outputs, test_depthGT)
            test_out_depth = compute_depth_map_validation_tof(test_fb, test_outputs_pr, device)
        else:
        ...
        max_0_80 = 80.0
        run_abs_rel_80, run_sq_rel_80, run_rmse_80, run_rmse_log_80, run_d1_80, run_d2_80, run_d3_80 = compute_errors_masked_train_max_dpth(test_out_depth_ups.to(device), test_depthGT.to(device), max_0_80)
        run_silog_80 = ScaleInvariantMSELogLoss_max_dpth(test_out_depth_ups.to(device), test_depthGT.to(device), 1.0, max_0_80)
        abs_rel_80  += run_abs_rel_80
        sq_rel_80   += run_sq_rel_80
        rmse_80     += run_rmse_80
        rmse_log_80 += run_rmse_log_80
        d1_80       += run_d1_80
        d2_80       += run_d2_80
        d3_80       += run_d3_80
        silog_80    += run_silog_80

        if TRACK_DISTRIBUTION == True:
            with open(log_d1, 'a') as f: 
                f.write(f"{run_d1_80},")
            with open(log_rmse, 'a') as f: 
                f.write(f"{run_rmse_80},")
            with open(log_silog, 'a') as f: 
                f.write(f"{run_silog_80},")
    ...
    with open(log_csv, 'w') as fcsv:
        ...
        fcsv.write(f"unlimited,{abs_rel:.3f},{sq_rel:.3f},{rmse:.3f},{rmse_log:.3f},{d1:.3f},{d2:.3f},{d3:.3f},{silog:.3f}\n")
    ...
    test_loss_80   /= num_batches
    abs_rel_80     /= num_batches
    sq_rel_80      /= num_batches
    rmse_80        /= num_batches
    rmse_log_80    /= num_batches
    d1_80          /= num_batches
    d2_80          /= num_batches
    d3_80          /= num_batches
    silog_80       /= num_batches

    # Print prediction statistics
    print(f"\nAVERAGE PREDICTION ACCURACY ON THE {size} IMAGES OF THE TEST SET (MAX DPTH 80 m)\nWITH RESPECT TO PROXY DEPTH MAPS:") 
    print(f"abs_rel = {abs_rel_80:.3f}")
    print(f"sq_rel = {sq_rel_80:.3f}")
    print(f"rmse = {rmse_80:.3f}")
    print(f"rmse_log = {rmse_log_80:.3f}")
    print(f"d1 = {d1_80:.3f}")
    print(f"d2 = {d2_80:.3f}")
    print(f"d3 = {d3_80:.3f}")
    print(f"silog = {silog_80:.3f}")

    with open(log_file, 'a') as f:
        f.write(f"AVERAGE PREDICTION ACCURACY ON THE {size} IMAGES OF THE TEST SET (MAX DPTH 80 m) WITH RESPECT TO PROXY DEPTH MAPS:\n")
        f.write(f"abs_rel = {abs_rel_80:.3f}, sq_rel = {sq_rel_80:.3f}, rmse = {rmse_80:.3f}, rmse_log = {rmse_log_80:.3f}, d1 = {d1_80:.3f}, d2 = {d2_80:.3f}, d3 = {d3_80:.3f}, silog = {silog_80:.3f}")
        f.write(f"\n")

    with open(log_csv, 'a') as fcsv:
        fcsv.write(f"80m,{abs_rel_80:.3f},{sq_rel_80:.3f},{rmse_80:.3f},{rmse_log_80:.3f},{d1_80:.3f},{d2_80:.3f},{d3_80:.3f},{silog_80:.3f}\n")
...
if VISUAL_TEST == 'Yes':
    ...
    with torch.no_grad():
        ...
        if SIMULATE_TOF == 'Y':
            if NUM_TOF_MINIMA == 1:
                depthfull = compute_depth_map_validation(fb.to(device), disp.to(device), device)
                disparity = simulate_tof_sensor_disparity(disp, 8, 8, depth.size()[-2], depth.size()[-1], -1, 'nearest')
                depth   = simulate_tof_sensor_ground_truth(depth, 8, 8, depth.size()[-2], depth.size()[-1], -1)
                depthGT = simulate_tof_sensor_ground_truth(depthGT, 8, 8, depthGT.size()[-2], depthGT.size()[-1], 0)
            elif NUM_TOF_MINIMA > 1:
                depthfull = compute_depth_map_validation(fb.to(device), disp.to(device), device)
                disparity = convert_depth_label_to_multi_target_disparity(depth.to(device), fb.to(device), NUM_TOF_MINIMA, device, 'fp32')
                depth   = convert_depth_label_to_multi_target_depth(depth.to(device), NUM_TOF_MINIMA, device, 'fp32').to(device)
                depthGT = convert_depth_label_to_multi_target_depth_GT(depthGT.to(device), NUM_TOF_MINIMA, device, 'fp32').to(device)
        else:
            disparity = disp
            depthfull = depth
        ...
        if SIMULATE_TOF == 'Y' and NUM_TOF_MINIMA > 1:
            dnn_out_bckp = torch.Tensor(dnn_out)
            data_transf  = PredictionMultitargetFinder(device, 'fp32')
            dnn_out      = data_transf(dnn_out.squeeze(1), depthGT)

        # Save depths from model
        dnn_out_depths = torch.zeros_like(dnn_out)

        # Compute prediction error
        abs_rel=0; sq_rel=0; rmse=0; rmse_log=0; d1=0; d2=0; d3=0; silog=0; neg_pred=0
        for smpl in range(6):
            # Save depth maps from model
            dnn_disp      = dnn_out[smpl].to(device)
            if SIMULATE_TOF == 'Y' and NUM_TOF_MINIMA > 1:
                dnn_disp = dnn_disp.unsqueeze(0)
                dnn_depth     = compute_depth_map_validation_tof(fb[smpl], dnn_disp, device)
            else: 
        ...
        for i in range(6):
            ...
            if SIMULATE_TOF == 'Y' and NUM_TOF_MINIMA > 1:
                i_dpthGT  = depthGT[i, 0, :, :].to('cpu')
                i_out     = dnn_out_bckp[i, :, :].squeeze(0).to('cpu') 
                i_dpth    = dnn_out_depths[i, 0, :, :].squeeze(0).to('cpu')
                i_dpth    = i_dpth.squeeze(0).to('cpu') 
                i_dispPR  = disp[i, :, :].squeeze(0).to('cpu')
                i_depthPR = depth[i, 0, :, :].squeeze(0).to('cpu')
                i_disp    = disparity[i, 0, :, :].squeeze(0).to('cpu')
                i_dpthfull = depthfull[i, :, :].squeeze(0).to('cpu')
            else:
                ...
                i_dpth    = i_dpth.squeeze(0).to('cpu') 
                i_disp    = disparity[i].squeeze(0).to('cpu')
                i_dpthfull = depthfull[i].squeeze(0).to('cpu')

            inv_mask_dpth = (i_dispPR == -1)
            inv_mask_disp = (i_dpthfull == -1)
            i_dispPR[inv_mask_dpth] = float('nan')
            i_dpthfull[inv_mask_disp] = float('nan')

            ax[i][0].title.set_text("Left Raw Image")
            ax[i][1].title.set_text("SGM Disparity")
            ax[i][2].title.set_text("SGM Depth")
            ax[i][3].title.set_text("Estimated Disparity")
            ax[i][4].title.set_text("Disparity Map (SGM)")
            ax[i][5].title.set_text("Depth Map (SGM)")
            ax[i][6].title.set_text("Estimated Depth")
            ax[i][7].title.set_text("Ground Truth Depth")
            ax[i][0].imshow(i_imgL)
            ax[i][1].imshow(i_dispPR, 'plasma')
            ax[i][2].imshow(i_dpthfull, 'plasma')
            ax[i][3].imshow(i_out, 'plasma')
            ax[i][4].imshow(i_disp, 'plasma')
            ax[i][5].imshow(i_depthPR, 'plasma')
            ax[i][6].imshow(i_dpth, 'plasma')
            ax[i][7].imshow(i_dpthGT, 'plasma')
