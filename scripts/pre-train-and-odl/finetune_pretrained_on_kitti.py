...
"""
THIS CODE IS SIMILAR TO THE ONE TO TRAIN uPyD-Net FROM SCRATCH ON LOW-RESOLUTION KITTI
...
import utils.kitti_dataloader as kdataloader
...
parser.add_argument( '--mini_kitti_highest_width_resolution', type=int, default=360)  # Needed to compute the rescaling of disparity maps in image reprojection loss
...
parser.add_argument( '--dataset_resolution', type=str, default='48x48')               # Resolution of the images in the mini-KITTI version, to select the right dataset subfolder (e.g., '48x48', '96x96')
...
parser.add_argument( '--upsample_strategy', type=str, default='bilinear')              # If proxy_disparity_resolution is "8x8", the disparity maps need to be upsampled for training (). 
...
parser.add_argument( '--percentage_kitti_trainset', type=int, default=100)             # Specify the percentage of training data in the KITTI train set to be used in training
...
# As 48 and 32 disparity maps are produced downsampling higher resolution disparity maps (e.g., 360 in width)
# this is needed to correctly scale the warping in the PhotometricReprojectionLoss 
MINIKITTI_MAX_WIDTH = args.mini_kitti_highest_width_resolution 
...
PERCENTAGE_KITTI_SAMPLES = args.percentage_kitti_trainset
...
if PERCENTAGE_KITTI_SAMPLES < 100:
    print(f"Fine-tuning on a subset of {trainset.__len__()} samples ({PERCENTAGE_KITTI_SAMPLES}% of the full mini-kitti)")

valset      = kdataloader.miniKITTI(MINIKITTI_PATH, transform=transform_val, set='val', resolution=kitti_source_resolution, normalize=normalize_imgs, flip_horizontally=False)
...
if TRACK_EPOCHWISE_TEST_ACC == 1:
    ...
    testset      = kdataloader.miniKITTI(MINIKITTI_PATH, transform=transform_test, set='test', resolution=kitti_source_resolution, normalize=normalize_imgs, flip_horizontally=False)
...
def change_lr_opt (optimizer, lr):
...
if resume_from_checkpoint == False:
    with open(filename, 'w') as file:
        ...
        if PERCENTAGE_KITTI_SAMPLES < 100:
            file.write(f'\nTRAINING ON A SUBSET OF {len(trainset)} SAMPLES ({PERCENTAGE_KITTI_SAMPLES}%)')
        file.write("\n--------------------------------------------------------")
...
for epoch in range(starting_epoch, epochs, 1):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader):
        ...
        if DATA_TYPE == 'fp32':
            ...
            imgR  = imgR.type(torch.HalfTensor)
            imgL  = imgL.to(device).type(torch.HalfTensor)
            imgR  = imgR.to(device).type(torch.HalfTensor)
            ...
            imgR  = imgR.type(torch.BFloat16Tensor)
            imgL  = imgL.to(device).bfloat16()
            imgR  = imgR.to(device).bfloat16()
            ...
            if DOWNSAMPLE_PREDICTION_OR_UPSAMPLE_LABEL == 'DOWNSAMPLE_PREDICTION':
                ...
                if NUM_TOF_MINIMA > 1:
                    with torch.no_grad():
                        ...
                        disp = convert_depth_label_to_multi_target_disparity(depth, fb, NUM_TOF_MINIMA, device, DATA_TYPE)
    ...
    with torch.no_grad():
        for test_data in valloader:
            ...
            if DATA_TYPE == 'fp32':
                ...
                val_imgR    = val_imgR.type(torch.HalfTensor)
                val_imgL    = val_imgL.to(device).half()
                val_imgR    = val_imgR.to(device).half()
                ...
                val_imgR    = val_imgR.type(torch.BFloat16Tensor)
                val_imgL    = val_imgL.to(device).bfloat16()
                val_imgR    = val_imgR.to(device).bfloat16()
            ...
            run_abs_rel, run_sq_rel, run_rmse, run_rmse_log, run_d1, run_d2, run_d3 = compute_errors_masked_train(val_out_depth_ups.to(device), val_depthGT.to(device))
            run_silog = ScaleInvariantMSELogLoss(val_out_depth_ups.to(device), val_depthGT.to(device), 1.0)
            abs_rel  += run_abs_rel
            sq_rel   += run_sq_rel
            rmse     += run_rmse
        ...
        if TRACK_EPOCHWISE_TEST_ACC == 1:
            ...
            with torch.no_grad():
                for test_data in testloader:
                    ...
                    """
                    TEST STATISTICS WITH RESPECT TO GROUND TRUTH
                    """
                    ...
                    test_Lap  += 0 # val_Lap_t
                    test_Lps  += 0 # val_Lps_t
                    test_La   += 0 # val_La_t
                    test_Lb   += 0 # val_Lb_t

                    #import pdb; pdb.set_trace()     

                    test_out_disp_ups  = transforms.functional.resize(img=test_outputs_filt, size=[test_depthGT.size()[-2], test_depthGT.size()[-1]], interpolation=transforms.InterpolationMode.NEAREST)
                    test_out_disp_ups  = test_out_disp_ups.squeeze(1)
                    test_out_depth_ups = compute_depth_map_test(test_fb, test_out_disp_ups, device, data_type=DATA_TYPE)
