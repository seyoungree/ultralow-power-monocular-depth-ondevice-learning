...
parser.add_argument('--startup_learning_rate', type=float, default=5e-4)
...
# All outputs prefixed with finetune_sde_ + run_id → never overwrites pre-trained files or other runs
statedict_dir             = args.saved_mdl_path
statedict_file            = f"{statedict_dir}/finetune_sde_{model_name}_{run_id}.pth"
statedict_info            = f"{statedict_dir}/finetune_sde_{model_name}_{run_id}.info"
...
filename                  = f"finetune_sde_train_log_{run_id}.txt"
out_file                  = f"finetune_sde_validation_outputs_{run_id}.txt"
...
trainset    = idataloader.miniIDSIADepth(
    IDSIADEPTH_PATH,
    transform=transform_train,
    set='train',
    normalize=normalize_imgs,
    flip_horizontally=HFLIP,
    train_subset_percentage=PERCENTAGE_IDSIADEPTH_SAMPLES
)
...
valset      = idataloader.miniIDSIADepth(
    IDSIADEPTH_PATH,
...
summary(net, (IM_CH_IN, IM_H_IN, IM_W_IN), batch_size=batch_size)   # model is still fp32 here
...
def change_lr_opt(optim_obj, lr):
    for param_group in optim_obj.param_groups:
        param_group['lr'] = lr

def get_lr_opt(optim_obj):
    for param_group in optim_obj.param_groups:
...
if resume_from_checkpoint and os.path.exists(running_checkpoint_folder):
    ...
    net.load_state_dict(torch.load(f"{running_checkpoint_folder}/finetune_sde_{model_name}.pth", map_location=device))
...
for epoch in range(starting_epoch, epochs, 1):
    ...
    for i, data in enumerate(trainloader):
        ...
        if args.probabilistic:
            ...
            mask_invalid = (disp >= 32000.0)
            disp[mask_invalid] = -1.0
    ...
    with torch.no_grad():
        for test_data in valloader:
            ...
            invalid_mask = (val_disp >= 32000.0)
            val_disp[invalid_mask] = -1.0
    ...
    with torch.no_grad():
        ...
        if checkpoint_model:
            ...
            with open(f"{checkpoint_folder}/ckpt.info", 'w') as log:
                ...
                log.write(f"{best_perf[0]},{best_perf[1]},{best_perf[2]},"
                          f"{best_perf[3]},{best_perf[4]},{best_perf[5]},"
                          f"{best_perf[6]},{best_perf[7]}\n")
            torch.save(net.state_dict(), f"{checkpoint_folder}/finetune_sde_{model_name}.pth")
        ...
        with open(f"{running_checkpoint_folder}/ckpt.info", 'w') as log:
            ...
            log.write(f"{val_loss},{abs_rel},{sq_rel},{rmse},"
                      f"{rmse_log},{d1},{d2},{d3}\n")
        torch.save(net.state_dict(), f"{running_checkpoint_folder}/finetune_sde_{model_name}.pth")

        # Final model info (overwritten every epoch with best summary)
        with open(statedict_info, 'w') as f:
            ...
            f.write(f"Learning rate: {initial_learning_rate} to {learning_rate} "
                    f"(scale every {scheduler_epochs_step} epochs)\n")
...
with torch.no_grad():
    ...
    if SAVE_MODEL:
        ...
        if os.path.exists(checkpoint_folder):
            shutil.copyfile(f"{checkpoint_folder}/finetune_sde_{model_name}.pth",
                            statedict_file)
            copy_tree(f"{tensorboard_folder}/",
                      f"{statedict_dir}/finetune_sde_tensorboard_data_{run_id}/")
        else:
